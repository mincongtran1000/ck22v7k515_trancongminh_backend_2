...
exports.create = async (req, res, next) => {
  ...
  try {
    ...
    console.log(error);
    return next(new ApiError(500, error.Object));
  }
};
...
exports.update = async (req, res, next) => {
  if (Object.keys(req.body).length === 0) {
    return next(new ApiError(400, "Data to update can not to empty"));
  }
...
exports.deleteAll = async (_req, res, next) => {
  try {
    ...
    return res.send({
      message: `${deletedCount} contract were deleted successfully`,
    });
  } catch (error) {
    return next(
